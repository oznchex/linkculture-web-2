...
export default function HomeExhibitionDetail() {
  const router = useRouter();
  const days = ['일', '월', '화', '수', '목', '금', '토'];
  const [selectedDay, setSelectedDay] = useState('수');

  // 전시·행사 카테고리만 필터링
  const exhibitionContents = dummyContents.filter(
    content => content.category === '전시·행사'
  );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-[80px]">
        <div className="space-y-4">
          {exhibitionContents.map((content, index) => (
            <div 
              key={index} 
